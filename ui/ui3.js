...
function update() {
	...
	node.append("text")
		.attr("dy", -3)
		.text(function (d) {return d.value && d.value.lad+":"+d.label;});
				...
				for (key in parent) {
					...
					if (child["#"]) {
						...
						linksByConnection.set(connectionId, {
							source: d3ParentNodeId, // nodesById.get(d3ParentNodeId),
							target: d3ChildNodeId, // nodesById.get(d3ChildNodeId),
						})
					}
				}
				fireUpdate()
